...
setup_backup_cron() {
    ...
    say "Writing daily backup cron → $cron"
    cat > "$cron" <<EOF
...
final_summary() {
    ...
    if curl -fsS -o /dev/null "http://127.0.0.1/"; then
        say "HTTP 200 on http://127.0.0.1/  ✓"
    else
        warn "Local curl did not return 200. Check journalctl -u caddy and journalctl -u php${PHP_VER}-fpm."
    fi
...
────────────────────────────────────────────────────────────────────
  ...
  Admin login  http://<host>/admin/login   (use the password you set)

  Code         $SRC_DIR
  Content      $SRC_DIR/content/posts/
  .env         $ENV_FILE   (mode 640, $APP_USER:www-data)
  Backups      $BACKUP_DIR   (daily 03:00, log in /var/log/lazyblog-backup.log)
  FPM pool     /etc/php/$PHP_VER/fpm/pool.d/lazyblog.conf
  Caddy site   /etc/caddy/Caddyfile

  Next steps
  • Point DNS at this host, then edit /etc/caddy/Caddyfile —
    replace "$LISTEN_ADDR" with your domain to enable HTTPS.
